...
class PhotoForm(Form):
    photo = FileField('上传图片', validators=[FileRequired(),FileAllowed(['jpg', 'png'],
                                                      '只能为图片')])

class NoticeForm(QuestionForm):
    category = SelectField('分类',
                           choices=[('社区公告', '社区公告')],
                           validators=[Required(message='分类不能为空')])


class DevelopForm(QuestionForm):
    title = StringField('标题:',
                        [Required(message='标题不能为空'),
                         Length(min=4,
                                max=36,
...
class SuggestForm(QuestionForm):
    category = SelectField('分类',
                           choices=[('反馈建议', '反馈建议')],
                           validators=[Required(message='分类不能为空')])

class MasterForm(QuestionForm):
    category = SelectField('分类',
                           choices=[('无', '无'),
                                    ('考研保研', '考研保研'),
                                    ('研究生活', '研究生活'),
...
class QuestionPostForm(QuestionForm):
    category = SelectField('分类',
