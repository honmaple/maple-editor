...
class Tags(db.Model):
    ...
    enname = db.Column(db.String(50), nullable=False)
    ...
    def __init__(self, enname, author):
        self.enname = enname
        self.author = author
        self.time = datetime.now()

    def __repr__(self):
        return '<Tags %r>' % self.enname
...
class Questions(db.Model):
    ...
    time = db.Column(db.DateTime, nullable=False)
    tags = db.relationship('Tags', secondary=tag_question,
                           lazy='dynamic',
                           backref="questions",
    ...
    collectors = db.relationship('User', secondary='collects',
                                 backref=db.backref(
                                      'collect_questions',
                                      lazy='dynamic'))
    ...
    board_f_id = db.Column(db.Integer, db.ForeignKey('board_f.id',
                                                     ondelete="CASCADE"))
    board_f = db.relationship('Board_F',
                              backref=db.backref('questions',
                                                 cascade='all,delete-orphan',
                                                 lazy='dynamic'))
    board_s_id = db.Column(db.Integer, db.ForeignKey('board_s.id',
                                                     ondelete="CASCADE"))
    board_s = db.relationship('Board_S',
                              backref=db.backref('questions',
                                                 cascade='all,delete-orphan',
                                                 lazy='dynamic'))
...
class Replies(db.Model):
    ...
    lovers = db.relationship('User', secondary='loves',
                             backref=db.backref(
                                  'love_replies',
                                  lazy='dynamic'))
...
class Collector(db.Model):
    ...
    def load_by_id(qid, uid):
        return Collector.query.filter_by(question_id=qid,
                                         user_id=uid).first()
...
class Lover(db.Model):
    ...
    def load_by_id(rid, uid):
        return Lover.query.filter_by(reply_id=rid,
                                     user_id=uid).first()
